...
struct _traj { // trajectory of an asteroid
    ...
    double distance;
    double travelTime;
    bool visible;  // might be able to remove
};
struct _signal {
    int antenna_id;
...
public:
    ...
    void generate_optimization(int numAsteroids, int numAntennas, int mutate, int popSize, double maxError); 
    ...
    /*
    vector<int> antennaAssignments;
    vector<bool> isRelocating;
    vector<double> transmittingPowers;
    vector<_aimpoint> antennaDirections;
    vector<double> imageInformations;
    vector<double> trajectoryInformations;
    vector<double> trajectoryScores; // Trajectory score for each asteroid, initialized in updateScore method
    vector<double> receviedSignalStrength;
    vector<double> outputPowerQueue;  // ArrayDeque used to find the received signal strength as time is a large variable
   
    vector<int> trajectoryIndices;*/
...
void AsteroidTracker::generate_optimization(int numAsteroids, int numAntennas, int mutate, int popSize, double maxError) {
    _pop pop;
    vector<_gn> genome;
    ...
    for (int i = 0; i < gene_groups; i++) {
        ...
        cout << "Antenna #: " << i << endl;
        cout << "targetID (2): " << round(create_64b_value(gene_1.gene, gene_1.min_value, gene_1.max_value)) << endl
            << "powerLevel (25%): " << round(create_64b_value(gene_2.gene, gene_2.min_value, gene_2.max_value)) << endl << endl;
...
double AsteroidTracker::calculateFitness(int targetID, double powerLevel) {
    ...
    /* 1. time - provided by nextCommand
     * 2. antennaID - provided by initialize
     * 3. antenna positions - provided by initialize
     * 4. antenna distances - [*] need to calculate
     * 5. asteroidID - provided by asteroidAppearance
     * 6. asteroid trajectory - provided by asteroidAppearance
     * 7. time signal sent (s) - [*] need to calculate
     * 8. transmission power (powerLevel) - [*] need to calculate
     * 9. distance to asteroid - [*] need to calculate
     * 10. initialTrajectoryInformation - provided by asteroidAppearance
     * 11. peakGain - provided by initialize
     * 12. appear (timeAppear):  time of asteroid appearance - - [*] need to calculate
     * 13. VISIBILITY CONSTRAINT - [*] need to calculate
     * 17. trajectoryInformation - [*] need to calculate
     * 19. subarray (antennaIDs targeting asteroidID) - [*] need to calculate
     * 21. absoluteMagnitude - provided within asteroidAppearance
     * 22. slewAngle - [*] need to calculate
     * 23. time to slew - [*] need to calculate
     * 24. albedo - provided within asteroidAppearance
     * 29. asteroid diameter - provided within asteroidAppearance
     * 30. ANTENNA RELOCATING CONSTRAINT - [*] need to calculate
     * 31. reflectivityMultiplier - provided by asteroidAppearance
     * 32. initialImageInformation - provided by asteroidAppearance
     * 33. effectiveSingalPowerReturn - [*] need to calculate
     * 34. energySpent - [*] need to calculate
     * 36. NEAR-FIELD CONSTRAINT - [*] need to calculate
     * 37. angle between beam and antenna - [*] need to calculate
     * 39. TARGET PROXIMITY CONSTRAINT - [*] need to calculate
     * 41. MAXIMUM TRANSMITTING POWER CONSTRAINT - [*] need to calculate
     * 45. scienceScoreMultiplier - provided by asteroidAppearance
     * 46. interpolateGain  - [*] need to calculate
     * 47. inducedNoise - [*] need to calculate
     * 48. totalNoise - [*] need to calculate
     * 49. informationRate - [*] need to calculate
     * 50. imageKnowledgeScore - [*] need to calculate
     * 51. trajectoryKnowledgeScore - [*] need to calculate
     * 52. finalScore - [*] need to calculate
     */ 
...
bool AsteroidTracker::isAscendingFromHorizon(int asteroidID, double viewTime) {
    ...
    asteroid = asteroids.at(asteroidID);
    pos1 = getAsteroidPosition(asteroidID, viewTime);
    pos2 = getAsteroidPosition(asteroidID, viewTime-180);
...
string AsteroidTracker::nextCommand(double currentTime) {    
    ...
    generate_optimization(numberOfAsteroids, numberOfAntennas, 10, 100, 0.1);
    /*
    double non_vis = 0;
    double vis = 0;
