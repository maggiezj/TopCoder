...
public:
    ...
    vector<_train> buildCategorySet(vector<_case> C);
    ...
    vector<double> calculateParameter(vector<_gn> genome, vector<_box> X);
...
void ChildStuntedness::generate_optimization(void) {
    ...
    int mutate = 5; // number of bits to randomly change in the genome
    int popSize = 100; // number of genomes in the population
    int maxCycle = 75; // number of cycles allowed with no improvement to population occurs
...
void ChildStuntedness::generate_optimization_2(_cases C, int mutate, int popSize, int maxCycle) {
    ...
    vector<double> w_typeI = cycle_population(C.typeI, C.T1, mutate, popSize, maxCycle, w_min, w_max, true);
    vector<double> w_typeII = cycle_population(C.typeII, C.T2, mutate, popSize, maxCycle, w_min, w_max, true);
    vector<double> w_typeIII = cycle_population(C.typeIII, C.T3, mutate, popSize, maxCycle, w_min, w_max, true);
    vector<double> w_typeIV = cycle_population(C.typeIV, C.T4, mutate, popSize, maxCycle, w_min, w_max, true);

    vector<double> d_typeI = cycle_population(C.typeI, C.T1, mutate, popSize, maxCycle, d_min, d_max, false);
    vector<double> d_typeII = cycle_population(C.typeII, C.T2, mutate, popSize, maxCycle, d_min, d_max, false);
    vector<double> d_typeIII = cycle_population(C.typeIII, C.T3, mutate, popSize, maxCycle, d_min, d_max, false);
    vector<double> d_typeIV = cycle_population(C.typeIV, C.T4, mutate, popSize, maxCycle, d_min, d_max, false);
...
vector<double> ChildStuntedness::cycle_population(vector<_case> type, vector<_box> Tx, int mutate, int popSize, int maxCycle, double min, double max, bool isW) {
    ...
    tX = buildCategorySet(type); // type -> {ID, cat, weight, duration}
    ...
    while (j < maxCycle) {
        ...
        pop = evolve_population(pop_r, mutate);
        j++;
    ...
    Y = calculateParameter(genome, Tx);
...
vector<double> ChildStuntedness::calculateParameter(vector<_gn> genome, vector<_box> Tx) {
    vector<double> Y;
    double x, y;
    _gn g;
    vector<double> N;
...
// determine the fitness of each member of the population
    ...
    for (int i = 0; i < population.size(); i++) {
        ...
        pop.fitness = 100.0 - abs(err/size_tX);
        new_population.push_back(pop);
    }    
    return new_population;
...
vector<_train> ChildStuntedness::buildCategorySet(vector<_case> Cs) {
    _train t;
    vector<_train> M;
    _case c;
 ...
 */ 
    ...
    tM = buildCategorySet(hCs);
    xM = buildCategorySet(nCs);
    //generate_optimization();
    generate_optimization_2(tS, 10, 250, 1000);
