/* ChildStuntedness v15 [Competition Sensitive]
 * TopCoder
...
public:
    ...
    /*void buildNNtopology(vector<int> neurons); 
    void feedForward(const vector<double> &inputVals);
    void backProp(const vector<double> &targetVals);
    void getResults(vector<double> &resultVals) const;
    double getRecentAverageError(void) const { return m_recentAverageError; }*/

    //primary interface to ChildStuntedness Class from main
    ...
    int numW, numD; // number of generated categories in weight and duration data
    vector<int> unique_trainingCategories, unique_testingCategories;
    double wt_min, wt_mean, wt_max, dur_min, dur_mean, dur_max;
...
_cases ChildStuntedness::splitCaseData(vector<_case> Cs) {
    ...
    S.B1 = buildCases(); // {age,sex,nutrition,ODV5-12,wt,duration} -> S.B1
    ...
    S.B2 = buildCases(); // {age,sex,nutrition,ODV5-12,wt,duration} -> S.B2
    clearParameters(); // clear -> {age,sex,nutrition,ODV5-12,wt,duration}
    
    loadParameters(S.typeIII); // Cs -> S.typeIII -> {age,sex,nutrition,ODV5-12,wt,duration}
    S.B3 = buildCases(); // {age,sex,nutrition,ODV5-12,wt,duration} -> S.B3
    clearParameters(); // clear -> {age,sex,nutrition,ODV5-12,wt,duration}
    
    loadParameters(S.typeIV); // Cs -> S.typeIII -> {age,sex,nutrition,ODV5-12,wt,duration}
    S.B4 = buildCases(); // {age,sex,nutrition,ODV5-12,wt,duration} -> S.B4
    clearParameters(); // clear -> {age,sex,nutrition,ODV5-12,wt,duration}
    ...
    if (outputStats) {
        ...
        for (int i = 0; i < S.B1.size(); i++) {
            cout << S.B1.at(i).mean << " ";
        }
        cout << endl;
        cout << "Type 2: " << S.typeII.size() << "\t";
        for (int i = 0; i < S.B2.size(); i++) {
            cout << S.B2.at(i).mean << " ";
        }
        cout << endl;
        cout << "Type 3: " << S.typeIII.size() << "\t";
        for (int i = 0; i < S.B3.size(); i++) {
            cout << S.B3.at(i).mean << " ";
        }
        cout << endl;
        cout << "Type 4: " << S.typeIV.size() << "\t";
        for (int i = 0; i < S.B4.size(); i++) {
            cout << S.B4.at(i).mean << " ";
        }
        cout << endl;
...
/*void ChildStuntedness::trainNN(void) {
    Analysis A;
    _box B;
    int trainingPass = 0;
    ...
    double NN_cycle_stop = NN_cycle_start + (1.0*1000000.0);
    ...
    while (clock() < NN_cycle_stop) {
        ...
        for (int j = 0; j < hCs.size(); j++) {
            ...
            for (int i = 0; i < T.size()-2; i++) {
                ...
                max = B.max;
                mean = B.mean;
                if ((val == 0) && ((i != 1) || (i != 2))) { // eradicate zeros from the data set
                    val = B.mean;
                }
            ...
            for (int i = T.size()-2; i < T.size(); i++) {
                ...
                max = B.max;
                mean = B.mean;
                if (val == 0) { // eradicate zeros from the data set
                    val = B.mean;
                }
                normVal = A.filter(val, mean, min, max);                
...
/*vector<_result> ChildStuntedness::testNN(void) {
    Analysis A;
    double NN_cycle_start = clock();
...
vector<double> ChildStuntedness::generateResult(vector<_result> resultVector) {
    ...
    for (int i = 0; i < resultVector.size(); i++) {
        outputVector.push_back(resultVector.at(i).weight);
        outputVector.push_back(resultVector.at(i).duration);
    }
    ...
    for (int i = 0; i < IDList.size(); i++) {
        ...
        duration = 0.0;
        int dataPts = 0;
        cout << "Cat: " << IDList.at(i) << " ---> ";
        for (int j = 0; j < resultVector.size(); j++) {
            if (IDList.at(i) == resultVector.at(j).ID) {
                cout << resultVector.at(j).weight << " ";
                weight += resultVector.at(j).weight;
                duration += resultVector.at(j).duration;
                dataPts++;
            }

        }
        cout << endl;
        outputVector.push_back(weight/(double) dataPts);
        outputVector.push_back(duration/(double) dataPts);
    }*/
...
_pop pop;
    ...
    int mutate = 1; // number of bits to randomly change in the genome
    int popSize = 1000; // number of genomes in the population
    int maxCycle = 10; // number of cycles allowed with no improvement to population occurs
...
// generate a population using with parameters defined to the objective function / competition
    ...
    vector<double> W;
    double W_min = wt_min; //0.504636;
    double W_max = wt_max; //0.946909;
   
    vector<double> D;
    double D_min = dur_min; //0.380281;
    double D_max = dur_max; //0.467912;

    for (int i = 0; i < population_size; i++) {
        genome.clear();
        
        W = create_values(wt_min, wt_max, numW);
        D = create_values(dur_min, dur_max, numD);
...
/*
void ChildStuntedness::buildNNtopology(vector<int> neurons) {
    topology.clear();
    
    cout << "Creating a <";
    for (int i = 0; i < neurons.size(); i++) {
        topology.push_back(neurons.at(i));
        cout << neurons.at(i);
        if (i < neurons.size()-1) {
                cout << " ";
        }
...
vector<double> ChildStuntedness::predict(vector<string> training, vector<string> testing) {
    ...
    */
    ...
    layer.push_back(14);
    layer.push_back(2);
    buildNNtopology(layer);*/

    // parse the training data
    parseData(training, true); // training -> hCs
    
    vector<double> wt_0, wt;
    vector<double> dur_0, dur;
    for (int i = 0; i < hCs.size(); i++) {
        wt_0.push_back(hCs.at(i).weight);
        dur_0.push_back(hCs.at(i).duration);
    }
    
    wt = vectorWithoutZeros(wt_0);
    dur = vectorWithoutZeros(dur_0);
    wt_min = A.min(vectorWithoutZeros(wt));
    wt_mean = A.average(vectorWithoutZeros(wt));
    wt_max = A.max(vectorWithoutZeros(wt));
    
    dur_min = A.min(dur);
    dur_mean = A.average(dur);
    dur_max = A.max(dur);

    // load vectors to train the NN
    //tS = splitCaseData(hCs); // generate a set of cases for each type (male/nutrition 1, male/nutrition 2, etc)
    T.clear();
    ...
    // train the NN and output parameters from training.
    //trainNN();
    ...
    numW = unique_trainingCategories.size();
    numD = numW;
    tM = buildTrainingSet();
    ...
    predictedData = lookupGAOptimizationTable();
    
    // perform the NN test
    //predictedData = testNN();
...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < 20; k=k+2) {
        cout << "#: " << (k/2) 
             ...
             << "\td: " << returnData.at(k+1)
             << endl;
        w.push_back(returnData.at(k));
        d.push_back(returnData.at(k+1));
    } 
